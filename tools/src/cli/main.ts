...
import { getDefaultLogDir } from "../services/logger";
import * as logTools from "../tools/logs";
import * as sessionTools from "../tools/session";
import * as taskTools from "../tools/tasks";
import type { TaskStatus } from "../types/tasks";
import { getErrorMessage, isTaskStatus } from "../utils/type-guards";
...
session
  ...
  .action(async (file: string) => {
    ...
    try {
      ...
      console.log(
        `${chalk.cyan("Start Time:")}    ${new Date(
          result.startTime
        ).toLocaleString()}`
      );
      console.log(
        `${chalk.cyan("End Time:")}      ${new Date(
          result.endTime
        ).toLocaleString()}`
      );
      ...
      console.log(
        `${chalk.cyan(
          "Cache Read:"
        )}          ${result.tokens.cacheRead.toLocaleString()}`
      );
      console.log(
        `${chalk.cyan(
          "Total:"
...
session
  ...
  .option("--max-output-length <chars>", "Maximum output length before truncation (default: no limit)", "10000")
    ...
    async (
      ...
      try {
        const markdown = await sessionTools.sessionToMarkdown(file, {
          ...
          maxOutputLength: options.maxOutputLength ? Number.parseInt(options.maxOutputLength, 10) : 10000,
    ...
    async (options: {
      ...
      try {
        ...
        for (const session of sessions) {
          ...
          try {
            const markdown = await sessionTools.sessionToMarkdown(
              session.filePath,
              {
                includeThinking: options.thinking !== false,
                includeToolDetails: options.toolDetails !== false,
            ...
            const outputFile = join(options.output, `${session.fileName}.md`);
            await writeFile(outputFile, markdown, "utf-8");

            exportSpinner.succeed(`Exported: ${session.fileName}.md`);
            successCount++;
          } catch (error) {
      ...
      if (options.interactive !== false) {
        ...
        readline.question("> ", async (answer: string) => {
          ...
          try {
            ...
            console.log(
              `${chalk.cyan("User Messages:")}      ${
                result.stats.userMessages
              }`
            );
            console.log(
              `${chalk.cyan("Assistant Messages:")} ${
                result.stats.assistantMessages
              }`
            );
            console.log(
              `${chalk.cyan("System Messages:")}    ${
                result.stats.systemMessages
              }`
            );
            console.log(
              `${chalk.cyan("Tool Uses:")}          ${result.stats.toolUses}`
  ...
  .action(async () => {
    ...
    try {
      ...
      if (stats.dateRange.start && stats.dateRange.end) {
        console.log(chalk.bold("\nDate Range:"));
        console.log(
          `  ${chalk.gray("From:")} ${new Date(
            stats.dateRange.start
          ).toLocaleString()}`
        );
        console.log(
          `  ${chalk.gray("To:")}   ${new Date(
            stats.dateRange.end
          ).toLocaleString()}`
        );
      }
...
tasks
  ...
  .action(async (taskId: string, options: { doc?: string }) => {
    ...
    try {
      ...
      console.log(
        `${chalk.cyan("Status:")}      ${getStatusColor(task.status)(
          task.status
        )}`
      );
