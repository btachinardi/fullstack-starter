...
session
  ...
  .option('-o, --output <dir>', 'Output directory (required)', './session-exports')
  .option('--no-thinking', 'Exclude thinking blocks')
  .option('--no-tool-details', 'Exclude tool input/output details')
  .option('--no-system-messages', 'Exclude system messages')
  .option(
    ...
    async (options: {
      ...
      try {
        ...
        for (const session of sessions) {
          const exportSpinner = ora(`Exporting: ${session.fileName}...`).start();

          try {
            const result = await sessionTools.sessionToEnrichedMarkdown(session.filePath, {
    ...
    try {
      ...
      if (options.interactive !== false) {
        ...
        readline.question('> ', async (answer: string) => {
          ...
          try {
            const result = await sessionTools.sessionInfo(selectedSession.filePath);
...
const tasks = program.command('tasks').description('Manage task documents (*.tasks.md files)');
  ...
  .action(
    async (options: {
      ...
      try {
        ...
        // Show summary
        const statusCounts = result.tasks.reduce<Record<string, number>>((acc, t) => {
