...
export function detectGenerationBoundary(
	...
	const sourcesMatch =
		/\/\/\s*Generated from:\s*\n((?:\/\/\s*-\s*.+\n)+)/i.exec(
			content.substring(startMatch.index, endMatch.index),
		);
	const sources: string[] = [];
	if (sourcesMatch) {
		// sourcesMatch[1] is string from captured group, possibly undefined
		...
		if (block) {
			...
			if (provider) {
				...
				if (output) {
					...
					const featuresMatch = /previewFeatures\s*=\s*\[([^\]]+)\]/s.exec(
						block,
					);
					...
					generators.push({ provider, output, previewFeatures });
				}
			}
		}
