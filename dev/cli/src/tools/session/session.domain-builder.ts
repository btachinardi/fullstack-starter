...
import type { EnrichedToolUse, SessionParser } from "./session.parser.js";
import type {
	AssistantEntry,
	SessionEntry,
	SystemEntry,
...
export class SessionDomainBuilder {
	...
	build(): EnrichedSession {
		...
		const mainThread: SessionThread = {
			...
			metadata: {
				...
				endTime:
					mainThreadMessages[mainThreadMessages.length - 1]?.timestamp || "",
			},
		};
	...
	private buildMainThreadMessages(
		...
		let currentCommandContext: CommandContext | undefined;
		...
		for (let i = 0; i < timelineEntries.length; i++) {
			...
			if (isUserEntry(entry)) {
				...
				const assistantMessages = this.buildAssistantMessages(
					entry,
					toolUseMap,
				);
	 ...
	 */
	private detectRequestInterrupted(
		entry: UserEntry,
	): RequestInterruptedMessage | null {
		const content = entry.message.content;
	...
	private detectSlashCommand(
		...
		const commandPrompt =
			typeof nextEntry.message.content === "string"
	...
	private buildSubagentThreads(): Map<string, SubagentThread> {
		...
		for (const [rootUuid, entries] of threadMap) {
			...
			const prompt =
				typeof firstEntry.message.content === "string"
			...
			const startTime =
				firstEntry && "timestamp" in firstEntry ? firstEntry.timestamp : "";
			const endTime =
				lastEntry && "timestamp" in lastEntry ? lastEntry.timestamp : "";
