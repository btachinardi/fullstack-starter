...
import type { MergedSchema, ParsedSchema } from "./prisma.types";
...
export async function mergeSchemas(
	...
	if (!appSchema) {
		throw new Error("Application schema is missing");
	}
	...
	if (!appSchema.datasource) {
		throw new Error("Application schema must have a datasource configuration");
	}

	const generator = appSchema.generators[0];
	if (!generator) {
		throw new Error(
			"Application schema must have at least one generator configuration",
		);
	}
...
function extractNativeModels(schema: ParsedSchema): string {
	...
	// Otherwise, extract all models and enums
	const parts: string[] = [];
	if (schema.enums.length > 0) {
		parts.push(schema.enums.join("\n\n"));
	}
	if (schema.models.length > 0) {
		parts.push(schema.models.join("\n\n"));
	}
	return parts.join("\n\n");
}
