...
| Windows developer | Onboards from Windows.                      | WSL2 flow documented and validated in CI; no “native Windows only” gaps.
...
* Capabilities:
  ...
  * Generate `.env.local`/`.env` from templates per workspace without overwriting existing values; warn for missing secrets.
