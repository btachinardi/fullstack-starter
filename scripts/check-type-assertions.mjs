...
function checkLineForAssertion(line) {
  ...
  while ((match = asPattern.exec(withoutStrings)) !== null) {
    ...
    const isAllowedExact = ALLOWED_ASSERTIONS.some((allowed) => {
      ...
      return typeAfterAs === allowedType || typeAfterAs.startsWith(`${allowedType} `);
    });

    // Check if it matches an allowed pattern (regex)
    const isAllowedPattern = ALLOWED_PATTERNS.some((pattern) => pattern.test(fullAssertion));
...
function main() {
  ...
  console.log(`  ${colors.gray}- Generic types (as T, as TValue)${colors.reset}`);
  console.log(`  ${colors.gray}- JSON.parse results with validation${colors.reset}`);
  console.log(`  ${colors.gray}- Global object types${colors.reset}`);
  console.log(`  ${colors.gray}- Error handling in catch blocks${colors.reset}`);
  console.log(`  ${colors.gray}- Third-party library integrations${colors.reset}`);

  console.log(`\n${colors.yellow}Recommendations:${colors.reset}`);
  console.log(`  ${colors.gray}1. Prefer type guards when possible${colors.reset}`);
  console.log(`  ${colors.gray}2. Use 'as unknown as T' for double casting (safer)${colors.reset}`);
  console.log(`  ${colors.gray}3. Add runtime validation for external data${colors.reset}`);
  console.log(`  ${colors.gray}4. Document why type assertions are necessary${colors.reset}`);
